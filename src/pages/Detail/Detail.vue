        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuAttr,index) in spuSaleAttrList" :key="spuAttr.id">
                ...
                <dd changepirce="0" :class="{active:attrValue.isChecked=='1'}"
                  v-for="(attrValue,index) in spuAttr.spuSaleAttrValueList"
                  :key="attrValue.id"
                  @click="changeActive(attrValue,spuAttr.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 进行路由跳转之前，需要发送请求，把购买的产品信息通过请求通知服务器，服务器进行相应的存储 -->
                <a style="cursor: pointer;" @click="addShopcar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      changeSkuNum(event){
        //监视用户数据改变是否合法，不合法则置为1，数值的上限设为99
        if(RegExp(/^[1-9]\d*$/).test(this.skuNum)){
          if(this.skuNum>99){
            this.skuNum = 99;
          }
        }else{
